            ...
            <div onClick={() => setShow(!show) } className={(!show ? 'hidden ' : '') + 'selected hand' }>
                <div>
                    <div className="token">
                        ...
                        { selected && (
                            <a target="_blank" href={explorerAddress(selected.contract_address + '.' + selected.value)} className="hash">
                        ...
                        {options.filter((x) => x.label.indexOf(search) > -1).map((option, index) => {
                            return <li onClick={() => {  onSelect(option) } } key={index}>
                                <div className="token">
                                    ...
                                    <a target="_blank" href={explorerAddress(option.contract_address + '.' + option.value)} className="hash">
